...
const buildHours = (hour) => {
    $('.forecast').append(`
        <div class="hour">
            <h1>${hour.hour}</h1>
            <img src="lib/images/${hour.icon}.png" id="hour-weather-icon">
...
const buildDays = (day) => {
    $('.forecast').append(`
        <div class="day">
            <h1>${day.day} <img src="lib/images/${day.icon}.png" id="day-weather-icon"></h1>
...
const buildWeatherCards = (forecast) => {
    ...
    `);
    forecast.hourly.slice(0, 10).forEach(hour => {
        buildHours(hour);
    });
